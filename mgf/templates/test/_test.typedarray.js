...
describe( 'typed-array mgf', function tests() {
	...
	it( 'should evaluate the  moment-generating function', function test() {
		...
		actual = mgf( actual, data, <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %> );
	...
	it( 'should return an empty array if provided an empty array', function test() {
		assert.deepEqual( mgf( new Int8Array(), new Int8Array(), <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %> ), new Int8Array() );
	});
