...
var Generator = yeoman.generators.Base.extend({
		...
		context[ 'parameterChecks' ] = this.parameters.map( function( p ) {
			...
			switch ( p.domain ) {
				case 'Real numbers':
					...
					s += '\t\t\treturn new TypeError( \'mgf()::invalid option. `' + p.name + '` parameter must be a number primitive. ';
				...
				case 'Positive real numbers':
					...
					s += '\t\t\treturn new TypeError( \'mgf()::invalid option. `' + p.name + '` parameter must be a positive number. ';
					s += 'Option: `\' + opts.' + p.name + ' + \'`.\' );\n';
					s += '\t\t}\n';
					s += '\t}';
				...
				case 'Non-negative real numbers':
					...
					s += '\t\t\treturn new TypeError( \'mgf()::invalid option. `' + p.name + '` parameter must be a non-negative number. ';
					s += 'Option: `\' + opts.' + p.name + ' + \'`.\' );\n';
					s += '\t\t}\n';
					s += '\t}';
				...
				case 'Non-negative integers':
					...
					s += '\t\t\treturn new TypeError( \'mgf()::invalid option. `' + p.name + '` parameter must be a non-negative integer. ';
					s += 'Option: `\' + opts.' + p.name + ' + \'`.\' );\n';
					s += '\t\t}\n';
					s += '\t}';
	...
	*/
	test: function() {
		...
		// R files
		this.copy( 'test/fixtures/_runner.jl', 'test/fixtures/runner.jl' );

		this.fs.copyTpl(
			this.templatePath( 'test/fixtures/_test.accessor.jl' ),
			this.destinationPath( 'test/fixtures/test.accessor.jl' ),
			context
		);
		this.fs.copyTpl(
			this.templatePath( 'test/fixtures/_test.array.jl' ),
			this.destinationPath( 'test/fixtures/test.array.jl' ),
			context
		);
		this.fs.copyTpl(
			this.templatePath( 'test/fixtures/_test.deepset.jl' ),
			this.destinationPath( 'test/fixtures/test.deepset.jl' ),
			context
		);
		this.fs.copyTpl(
			this.templatePath( 'test/fixtures/_test.matrix.jl' ),
			this.destinationPath( 'test/fixtures/test.matrix.jl' ),
			context
		);
		this.fs.copyTpl(
			this.templatePath( 'test/fixtures/_test.number.jl' ),
			this.destinationPath( 'test/fixtures/test.number.jl' ),
			context
		);
		this.fs.copyTpl(
			this.templatePath( 'test/fixtures/_test.partial.jl' ),
			this.destinationPath( 'test/fixtures/test.partial.jl' ),
			context
		);
		this.fs.copyTpl(
			this.templatePath( 'test/fixtures/_test.typedarray.jl' ),
			this.destinationPath( 'test/fixtures/test.typedarray.jl' ),
			context
		);
	}, // end METHOD test()
