...
function pdf( x, options ) {
	...
	if ( isMatrixLike( x ) ) {
		...
		return pdf5( out, x, <%= optsArguments %> );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, <%= optsArguments %> );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, <%= optsArguments %>, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, <%= optsArguments %>, opts.accessor );
		}
		return pdf2( out, x, <%= optsArguments %> );
	}
	return NaN;
} // end FUNCTION pdf()
