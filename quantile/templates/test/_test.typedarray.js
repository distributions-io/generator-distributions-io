...
describe( 'typed-array quantile', function tests() {
	...
	it( 'should evaluate the quantile function of the <%= distribution %> distribution', function test() {
		...
		actual = quantile( actual, data, <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %> );
	...
	it( 'should return an empty array if provided an empty array', function test() {
		assert.deepEqual( quantile( new Int8Array(), new Int8Array(), <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %> ), new Int8Array() );
	});
