...
describe( 'accessor quantile', function tests() {
	...
	it( 'should evaluate the quantile function of the <%= distribution %> distribution using an accessor', function test() {
		...
		actual = quantile( actual, data, <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %>,getValue );
