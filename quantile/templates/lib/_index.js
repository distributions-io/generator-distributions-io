...
function quantile( p, options ) {
	...
	if ( isNumber( p ) ) {
		return quantile1( p, <%= optsArguments %> );
	}
	if ( isMatrixLike( p ) ) {
		if ( opts.copy !== false ) {
		...
		return quantile5( out, p, <%= optsArguments %> );
	}
	if ( isTypedArrayLike( p ) ) {
		...
		return quantile6( out, p, <%= optsArguments %> );
	}
	if ( isArrayLike( p ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return quantile4( p, <%= optsArguments %>, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return quantile3( out, p, <%= optsArguments %>, opts.accessor );
		}
		return quantile2( out, p, <%= optsArguments %> );
	}
	return NaN;
