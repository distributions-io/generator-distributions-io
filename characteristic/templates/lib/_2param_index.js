...
function mean( <%- parameterArguments %>, options ) {
	...
	if ( arguments.length < 2 ) {
		throw new Error( 'mean():: Insufficient number of arguments. Both `<%=x%>` and `<%=y%>` arguments have to be supplied' );
	}
	...
	// Handle typed-array output...
		...
		(<%=x%>Type === 'typedarray' && <%=y%>Type === 'number') || (<%=x%>Type === 'number' && <%=y%>Type === 'typedarray')
	) {
		dt = opts.dtype || 'float64';
		ctor = ctors( dt );
		if ( ctor === null ) {
