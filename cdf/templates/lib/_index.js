...
function cdf( x, options ) {
	...
	if ( isMatrixLike( x ) ) {
		...
		return cdf5( out, x, <%= optsArguments %> );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, <%= optsArguments %> );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, <%= optsArguments %>, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, <%= optsArguments %>, opts.accessor );
		}
		return cdf2( out, x, <%= optsArguments %> );
	}
	return NaN;
} // end FUNCTION cdf()
