...
describe( 'typed-array cdf', function tests() {
	...
	it( 'should evaluate the <%= distribution %> cdf', function test() {
		var data, actual, expected, i;

		data = new Float64Array( validationData.data );
		actual = new Float64Array( data.length );

		actual = cdf( actual, data, <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %> );
	...
	it( 'should return an empty array if provided an empty array', function test() {
		assert.deepEqual( cdf( new Int8Array(), new Int8Array(), <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %> ), new Int8Array() );
	});
