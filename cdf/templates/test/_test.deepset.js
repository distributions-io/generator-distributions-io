...
describe( 'deepset cdf', function tests() {
	...
	it( 'should compute the <%= distribution %> cdf and deep set', function test() {
		var data, expected, i;
		...
		data = cdf( data, <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %>, 'x' );
		...
		data = cdf( data, <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %>, 'x/1', '/' );
		expected = validationData.expected
			.map( function( d ) {
				return d === 'Inf' ? Infinity : d;
			})
	...
	it( 'should return an empty array if provided an empty array', function test() {
		assert.deepEqual( cdf( [], <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %>, 'x' ), [] );
		assert.deepEqual( cdf( [], <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %>, 'x', '/' ), [] );
	});
	...
	it( 'should handle non-numeric values by setting the element to NaN', function test() {
		...
		actual = cdf( data, <%= parameters.map( function( p ) { return p.name} ).join( ', ' ) %>, 'x' );
